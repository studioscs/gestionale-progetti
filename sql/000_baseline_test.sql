...
create table if not exists public.project_fasi(
...
create table if not exists public.project_sottofasi(
...
-- dati di partenza (idempotenti: il file si puo' rieseguire senza errori)
...
on conflict (id) do nothing;
-- tasks non ha un id fisso: si guarda il titolo per non duplicare la riga
insert into public.tasks(title,project_id,status)
select 'Task preesistente','33333333-3333-3333-3333-333333333333','in_corso'
where not exists (select 1 from public.tasks where title = 'Task preesistente');
